...
public class S_Email
{
        ...
        string p_motDePasseTemporaire, string p_nomEntrepriseDirigeant)
    {
        ...
        {
            _logger.LogWarning("BREVO_API_KEY manquante, email création compte non envoyé à {Email}", p_emailDestinataire);
            return false;
        }
...
<body style=""font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;"">
    ...
    <div style=""background: #fff; padding: 24px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 8px 8px;"">
        ...
        try
        {
            ...
            {
                _logger.LogInformation("Email création compte envoyé à {Email}", p_emailDestinataire);
                return true;
            }
            else
            {
                var _body = await _reponse.Content.ReadAsStringAsync();
                _logger.LogError("Erreur Brevo création {Status} pour {Email} : {Body}", _reponse.StatusCode, p_emailDestinataire, _body);
                return false;
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Exception envoi email création à {Email}", p_emailDestinataire);
            return false;
        }
    }

    // Invitation à rejoindre une entreprise, pour un utilisateur qui a DÉJÀ un compte
    public async Task<bool> EnvoyerInvitationCollaborateur(string p_emailDestinataire, string p_prenom, string p_nomEntreprise)
    {
        var _apiKey = Environment.GetEnvironmentVariable("BREVO_API_KEY") ?? _config["Brevo:ApiKey"];
        var _fromEmail = Environment.GetEnvironmentVariable("SMTP_FROM") ?? _config["Brevo:FromEmail"] ?? "contact@codebtpsecure.cloud";
        var _fromName = Environment.GetEnvironmentVariable("SMTP_FROM_NAME") ?? _config["Brevo:FromName"] ?? "KEYDO";
        ...
        if (string.IsNullOrEmpty(_apiKey))
        {
            _logger.LogWarning("BREVO_API_KEY manquante, invitation collaborateur non envoyée à {Email}", p_emailDestinataire);
            return false;
        }
...
<body style=""font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;"">
    ...
    <div style=""background: #fff; padding: 24px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 8px 8px;"">
        ...
        try
        {
            ...
            if (_reponse.IsSuccessStatusCode)
            {
                _logger.LogInformation("Invitation collaborateur envoyée à {Email}", p_emailDestinataire);
                return true;
            }

            var _body = await _reponse.Content.ReadAsStringAsync();
            _logger.LogError("Erreur Brevo invitation collaborateur {Status} pour {Email} : {Body}", _reponse.StatusCode, p_emailDestinataire, _body);
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Exception envoi invitation collaborateur à {Email}", p_emailDestinataire);
            return false;
        }
    }
...
<body style=""font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;"">
    ...
    public async Task<bool> EnvoyerCodeTiers(string p_emailDestinataire, string p_valeurCode, string p_nomEntreprise, string p_numeroCommande)
    {
        ...
        if (string.IsNullOrEmpty(_apiKey))
        {
            _logger.LogWarning("BREVO_API_KEY manquante, code tiers non envoyé à {Email}", p_emailDestinataire);
            return false;
        }
...
<body style=""font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;"">
    ...
    <div style=""background: #fff; padding: 24px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 8px 8px;"">
        ...
        <p style=""color: #666; font-size: 14px;"">
            Présentez ce code au fournisseur pour valider l'achat. Ce code est valable <strong>24 heures</strong> et utilisable <strong>une seule fois</strong>.
        </p>
        ...
        try
        {
            ...
            if (_reponse.IsSuccessStatusCode)
            {
                _logger.LogInformation("Code tiers envoyé à {Email}", p_emailDestinataire);
                return true;
            }
            else
            {
                var _body = await _reponse.Content.ReadAsStringAsync();
                _logger.LogError("Erreur Brevo code tiers {Status} pour {Email} : {Body}", _reponse.StatusCode, p_emailDestinataire, _body);
                return false;
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Exception envoi code tiers à {Email}", p_emailDestinataire);
            return false;
        }
    }
