...
public class S_Invitation
{
    ...
    public async Task<List<DTO_InvitationAffichage>> ObtenirInvitations(int p_salarieId)
    {
        var _liens = await _daoEntreprise.ObtenirInvitationsParSalarie(p_salarieId);
        return _liens.Select(l => new DTO_InvitationAffichage
        {
            Id = l.Id,
            NomEntreprise = l.Entreprise.Nom,
            SiretEntreprise = l.Entreprise.Siret,
            NomPatron = l.Entreprise.Patron.Nom,
            PrenomPatron = l.Entreprise.Patron.Prenom,
            DateAjout = l.DateAjout,
            Statut = l.StatutInvitation
        }).ToList();
    }

    public async Task<(bool Succes, string Message)> Accepter(int p_lienId, int p_salarieId)
    {
        var _lien = await _daoEntreprise.ObtenirLienParId(p_lienId);
        if (_lien == null || _lien.SalarieId != p_salarieId)
        ...
        _logger.LogInformation("Salarié {SalarieId} a accepté l'invitation pour l'entreprise {Entreprise}",
            p_salarieId, _lien.Entreprise.Nom);
        return (true, $"Vous avez rejoint l'entreprise {_lien.Entreprise.Nom}.");
    }

    public async Task<(bool Succes, string Message)> Refuser(int p_lienId, int p_salarieId)
    {
        var _lien = await _daoEntreprise.ObtenirLienParId(p_lienId);
        if (_lien == null || _lien.SalarieId != p_salarieId)
            return (false, "Invitation non trouvée.");
        ...
        _logger.LogInformation("Salarié {SalarieId} a refusé l'invitation pour l'entreprise {Entreprise}",
            p_salarieId, _lien.Entreprise.Nom);
        return (true, $"Invitation de {_lien.Entreprise.Nom} refusée.");
    }

    public async Task<(bool Succes, string Message)> Quitter(int p_lienId, int p_salarieId)
    {
        var _lien = await _daoEntreprise.ObtenirLienParId(p_lienId);
        if (_lien == null || _lien.SalarieId != p_salarieId)
        ...
        await _daoCode.RevoquerCodesParSalarieEtEntreprise(p_salarieId, _lien.EntrepriseId);

        _logger.LogInformation("Salarié {SalarieId} a quitté l'entreprise {Entreprise}, codes révoqués",
            p_salarieId, _lien.Entreprise.Nom);
